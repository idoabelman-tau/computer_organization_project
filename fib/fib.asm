...
LOOP:
	sub $t0, $s0, $imm, -2 # get the memory location for the fibo number 2 before the current (f[n-2])
	lw $t1, $t0, $zero, 0 # load f[n-2] into t1
	sub $t0, $s0, $imm, -1 # get the memory location for the fibo number 1 before the current (f[n-1])
	...
	bgt $imm, $s0, $zero, LOOP # loop back if no overflow happened in $s0 (it's greater than 0)
END:
